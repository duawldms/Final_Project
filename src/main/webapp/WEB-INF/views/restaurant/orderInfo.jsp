...
<div class="order_cancel_wrap">
	...
	<table class="order_cancel_table" style="border-collapse: collapse">
 		...
 		<tbody>
			...
			<tr>
				<td colspan="3"><hr>총 결제 금액 : ${requestScope.list.or_totalcost }</td>
			</tr>
			<tr>
				<td colspan="3"><hr>주문번호 : ${requestScope.list.or_num }<hr></td>
			</tr>
			<tr>
				<td colspan="3">주문 시간 : ${requestScope.list.or_regdate }<hr></td>
			</tr>
			<c:if test="${vo.or_request != null }">
				<tr>
					<td colspan="3">요청 사항 : ${requestScope.list.or_request }<hr></td>
				</tr>
			</c:if>
			<tr>
				<td colspan="3">
					<c:choose>
						<c:when test="${requestScope.list.or_status == 1}">
